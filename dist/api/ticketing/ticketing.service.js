...
let TicketingService = class TicketingService {
    ...
    async ticketIssueRequest(header, bookingUId, payment) {
        ...
        if (booking?.status != 'Hold') {
            ...
            throw new common_1.NotAcceptableException('Ticket is non refundable cannot make partial payment');
        }
        ...
        if (payment === 'partial') {
            ...
            if (daysDifference < 4) {
                throw new common_1.NotAcceptableException("Partial Paymnet cannot Performed when flight date is less than 4 Days");
            }
            ...
            if (agentLedgerValue <= payableamount) {
                ...
                if (!ppsRes?.id) {
                    throw new common_1.NotImplementedException('Partial data is not yet implemented');
                }
