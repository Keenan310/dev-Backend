...
let AlhindAPI = class AlhindAPI {
    ...
    async flights(agent, flightDto) {
        ...
        try {
            const response = await axios_1.default.post(`https://b2b.keenantravel.com/api.php`, data, { headers });
    ...
    async flightUtils(result, agentdata, flighDto) {
        if (result?.Journy?.FlightOptions?.length > 0) {
            ...
            for (const flights of AllFareWithPrice) {
                ...
                const equivalentAmount = flights.PriceBreakDown?.AprxTotalBaseFare * converstionrate;
                const Taxes = flights.PriceBreakDown?.AprxTotalTax * converstionrate;
                let TotalFare = Math.ceil(flights.PriceBreakDown?.TotalAmount * converstionrate);
                ...
                FlightItenary.push({
                    ...
                    Cabinclass: Class,
                    Currency: currency,
                    BaseFare: equivalentAmount * converstionrate,
                    Taxes: Taxes * converstionrate,
                    NetFare: NetFare,
                    GrossFare: TotalFare,
